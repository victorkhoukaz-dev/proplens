...
(() => {
  ...
  const trackerGameFilter = (() => {
    ...
    select.id = 'tracker-game-filter';
    select.setAttribute('aria-label', 'Filter by matchup');
  ...
  function gameFilterOption(team, opponent) {
    const teams = [team, opponent].map(value => String(value || '').trim().toUpperCase()).filter(Boolean);
    if (teams.length !== 2) return null;
    return { key: teams.slice().sort().join('|'), label: `${teams[0]} vs ${teams[1]}` };
  }
  function activityForBet(bet) { const game = gameFilterOption(bet.team, bet.opponent); return { ...bet, activity_type: 'straight', activity_games: game ? [game.key] : [], game_season: bet.result_identity?.season ?? null, game_week: bet.result_identity?.week ?? null, activity_search: `${bet.description || ''} ${bet.player_name} ${bet.market} ${bet.category || ''} ${bet.position || ''} ${bet.team || ''} ${bet.opponent || ''}`, activity_sort_name: bet.description || bet.player_name }; }
  ...
  function activityForParlay(parlay) { const context = parlayWeekContext(parlay); return { ...parlay, activity_type: 'parlay', activity_games: parlay.legs.map(leg => gameFilterOption(leg.team, leg.opponent)?.key).filter(Boolean), game_season: context.season, game_week: context.week, activity_search: `${parlay.description || ''} ${parlay.legs.map(leg => `${leg.description || ''} ${leg.player_name} ${leg.market} ${leg.team || ''} ${leg.opponent || ''}`).join(' ')}`, activity_sort_name: parlay.description || parlay.legs[0]?.player_name || 'Parlay' }; }
  ...
  function populateGameFilter(activity) {
    const currentGame = trackerGameFilter.value, labels = new Map();
    reportFilteredActivity(activity).forEach(item => {
      const legs = item.activity_type === 'parlay' ? item.legs : [item];
      legs.forEach(leg => { const game = gameFilterOption(leg.team, leg.opponent); if (game && !labels.has(game.key)) labels.set(game.key, game.label); });
    });
    const games = [...labels.entries()].sort(([, left], [, right]) => left.localeCompare(right));
  ...
  function filteredContextLabel() {
    const labels = [reportContextLabel()];
    if (trackerActivityFilter.value !== 'all') labels.push(selectedLabel(trackerActivityFilter));
    if (trackerStatusFilter.value !== 'all') labels.push(selectedLabel(trackerStatusFilter));
    if (trackerTypeFilter.value !== 'all') labels.push(selectedLabel(trackerTypeFilter));
  ...
  function scopedBonusCreditSources(sources) {
    const season = trackerSeasonFilter.value, week = trackerWeekFilter.value;
  ...
  function render(data) {
    ...
    renderOverall({ summary: s, include_parlays: trackerIncludeParlays.checked, outcome_only: nonPerformanceStatus });
  }
  ...
  function parlayPreviewMarkup(parlay) {
    ...
    const legs = (parlay.legs || []).map(leg => {
      const selection = [leg.player_name, marketLabel(leg.market), leg.line].filter(value => value !== undefined && value !== null && value !== '').join(' · ');
      const detail = leg.status === 'proposal' ? `Actual: ${leg.actual_stat} ${leg.stat_label || ''}` : leg.message;
      const outcome = leg.proposed_result ? leg.proposed_result[0].toUpperCase() + leg.proposed_result.slice(1) : leg.status === 'manual_required' ? 'Manual' : leg.status === 'game_not_final' || leg.status === 'stats_unavailable' ? 'Waiting' : 'Review';
      return `<div class="result-preview-row parlay-result-preview-row"><div><strong>${escapeHtml(selection)}</strong><small>${escapeHtml(detail || '')}</small></div><span class="result-preview-outcome ${escapeHtml(leg.proposed_result || (leg.status === 'waiting' ? 'waiting' : 'review'))}">${escapeHtml(outcome)}</span></div>`;
    }).join('');
    return `<section class="result-preview-parlay"><div class="result-preview-parlay-heading"><div><strong>${escapeHtml(parlay.description)}</strong><small>${escapeHtml(parlay.message || '')}</small></div><span class="result-preview-outcome ${escapeHtml(parlay.proposed_result || (parlay.status === 'waiting' ? 'waiting' : 'review'))}">${escapeHtml(suggestion)}</span></div>${legs}</section>`;
  }
  function renderResultPreview(data) {
    ...
    const parlayProposals = (data.parlays || []).filter(item => item.status === 'proposal');
    const proposed = proposedItems.length + parlayProposals.length;
  ...
  function showSavedEvaluation(bet) {
    const projection = Number(bet.projection_mean), probability = Number(bet.model_win_probability), fairOdds = Number(bet.model_fair_decimal), ev = Number(bet.expected_value_pct), odds = Number(bet.decimal_odds);
    const source = bet.source_context || {}, identity = bet.result_identity || {};
    const sourceLabel = source.label || identity.projection_snapshot_label || 'Imported projection set';
    const seasonWeek = [identity.season ?? source.season, identity.week ?? source.week].every(Number.isInteger) ? ` · NFL ${identity.season ?? source.season} Week ${identity.week ?? source.week}` : '';
    $('#saved-evaluation-title').textContent = 'Original evaluation';
    $('#saved-evaluation-summary').textContent = `${bet.player_name} · ${bet.side_label} ${bet.line} · ${marketLabel(bet.market)}. This is the model snapshot saved when you recorded this bet.`;
    $('#saved-evaluation-source').textContent = `Projection source: ${sourceLabel}${seasonWeek}`;
