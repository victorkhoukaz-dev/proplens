...
- **Mixed evaluated/manual shortcut — retired after workflow testing:** the compact free-text **Add manual leg** field was removed from the evaluated Parlay slip because it duplicated the richer manual-parlay workflow. Use **Track manual parlay** for guided player/market legs or unrestricted free text; the evaluated Parlay slip now contains evaluated legs only.
- **Phase 3C.1A — implemented and user-tested:** reuse the existing nflverse final-stat checker for every eligible pending parlay leg and show a separate, preview-only per-leg result panel. A whole-parlay win needs every leg to win; one supported final losing leg can safely suggest a parlay loss even if another leg remains manual-review. Pushes, voids, free-text, unsupported, incomplete, and ambiguous legs stay manual. This preview never changes a tracked parlay.
- **Phase 3C.1B — implemented and user-tested:** the Bet tracker’s main **Check results** action runs one combined, preview-only check for straight bets and parlays and displays both sections together in the Bet tracker. A season’s nflverse file is refreshed at most once per combined check, then reused by matching parlay legs.
- **Phase 3C.1C — implemented and user-tested:** display an explicit **Confirm Win** or **Confirm Loss** beside an actual pending parlay only when its latest checked whole-parlay suggestion is unambiguous. Confirmation rechecks the saved parlay against cached final stats before settlement and saves the nflverse leg evidence. Pushes, voids, cash-outs, and any manual/ambiguous result remain on normal manual settlement.
- **Unified Tracker — implemented and user-tested:** the Bet Tracker is now the single user-facing ledger for straight bets and parlays. Its collapsible parlay rows expose the same settlement, cash-out, adjustment, edit, cancel, delete, and result-confirmation controls while keeping each leg readable on demand. Each row retains its own leg toggle, and one tracker-level control expands or collapses every parlay currently visible under the active filters. The manual-parlay entry opens from the Parlay slip. The redundant Parlay Tracker navigation entry and standalone parlay-results surface have been retired; activity filters provide **All activity**, **Straight bets**, and **Parlays** views.
...
- **Phase 4F — implemented and expanded:** fully identified manual player props can receive the existing **preview-only** nflverse result suggestions. Tested markets are passing yards, rushing yards, **rushing attempts**, receiving yards, receptions, plus Anytime TD **win suggestions only**. The bet must contain player, team, opponent, season, week, supported market, and an exact Over/Under line (or Anytime TD Yes). The user must still explicitly confirm every proposed result; custom, defensive, game, incomplete, and untested markets remain **Manual settlement required**.
- **Rushing attempts — implemented:** FantasyPoints `Rush ATT` projections are available in the evaluator, evaluated-parlay labels, manual straight bets, manual parlay legs, later manual-bet evaluations, result previews, and the later mean-accuracy report. Attempts use the count-market negative-binomial path with an initial dedicated dispersion prior; calibration will revisit that assumption once enough final results exist.
...
- **Implemented and user-tested:** show straight bets and parlays together in the Bet tracker and include both in the overall profit and ROI totals.
- Keep a parlay as one labelled, collapsible activity row; reveal its individual legs only when requested and never duplicate those legs as separate wagers.
...
- Settle, adjust, edit, and check results for both straight bets and parlays from the unified Bet tracker.
