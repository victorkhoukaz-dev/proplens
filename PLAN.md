...
- **Phase 3C.1B — implemented, awaiting user testing:** the Bet tracker’s main **Check results** action now runs one combined, preview-only check for straight bets and parlays and displays both sections together in the Bet tracker. A season’s nflverse file is refreshed at most once per combined check, then reused by matching parlay legs. The dedicated Parlay tracker action remains as a focused detail view.
- **Phase 3C.1C — implemented, awaiting user testing:** display an explicit **Confirm Win** or **Confirm Loss** beside an actual pending parlay only when its latest checked whole-parlay suggestion is unambiguous. Confirmation rechecks the saved parlay against cached final stats before settlement and saves the nflverse leg evidence. Pushes, voids, cash-outs, and any manual/ambiguous result remain on normal manual settlement.
...
- **Phase 4F — implemented, awaiting user testing:** fully identified manual player props can receive the existing **preview-only** nflverse result suggestions. The initial tested markets are passing yards, rushing yards, receiving yards, receptions, plus Anytime TD **win suggestions only**. The bet must contain player, team, opponent, season, week, supported market, and an exact Over/Under line (or Anytime TD Yes). The user must still explicitly confirm every proposed result; custom, defensive, game, incomplete, and untested markets remain **Manual settlement required**.
